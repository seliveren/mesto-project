...
import {getInitialCards, postedCard, putLike, deletedLike, deletedCard} from './api.js'
...
getInitialCards()
  .then((res) => {
    res.forEach(el => {
      const card = addPlace(el.name, el.link, el.alt, el._id, el.owner._id, el.likes, el.likes.length);
      insertCard(card);
    })
  })
  .catch((err) => {
...
function addPlace(placeNameValue, placeLinkValue, placeAltValue, id, ownerId, placeLikes, placeLikesLength) {
  ...
  cardElement.setAttribute('data-id', `${id}`);
    ...
    if (hasMyId) {
      cardElement.querySelector('.button_category_like').classList.add('button_category_like-active');
    }

  //добавление корзины только у моей карточки
  if (ownerId === '3c95ff7236bc20de41907267'){
    insertMarkup(cardElement, createDeleteMarkup());
    cardElement.querySelector('.button_category_delete').addEventListener('click',
      function(e) {
  ...
  cardElement.querySelector('.button_category_like').addEventListener('click', function(e) {
    let id = e.target.closest('.places__place').dataset.id;

      if (!(cardElement.querySelector('.button_category_like').classList.contains('button_category_like-active')) && !hasMyId) {
        addLikes(id);
        cardElement.querySelector('.button_category_like').classList.add('button_category_like-active');
        cardElement.querySelector('.like-counter').textContent = placeLikesLength + 1;
      } else {
        deleteLikes(id);
        cardElement.querySelector('.button_category_like').classList.remove('button_category_like-active');
        cardElement.querySelector('.like-counter').textContent = placeLikesLength;
      }
  });
...
function createCard(newCard) {
  postedCard(newCard)
    ...
    .finally(res => {renderLoading(false, formAddCard)})
}
